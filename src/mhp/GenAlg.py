...
class GenAlg:
    def __init__(
            self,
            time_limit_s =  120, # in seconds, for 0 one run with other conditions, for < 0 run without time limit
            stop_on_first = True,
            ...
            parent_selection_type = 'tournament', # sss : steady_state_selection, rws : roulette_wheel_selection, sus : stochastic_universal_selection, random, tournament, rank
        ...
        self.time_limit                = time_limit_s * (10 ** 9)
    ...
    def run(self, dn: DroneNet) -> GASolution:
        ...
        while ((self.time_limit < 0) or (self.time_limit > executionTime)) and ((not self.stop_on_first) or (solutionFitness < 0)):
            run_time = perf_counter_ns()
            ga = GA(
            ...
            runs.append((run_time - perf_counter_ns(), currentSolutionFitness, currentSolution, currentSolutionFitnessByFitness, currentSolutionByFitness))
