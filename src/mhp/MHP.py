...
class DroneNet:
    ...
    @staticmethod
    def createRandomArea(size, drone_number, max_bandwidth, net_bandwidth_percentage, max_range_percentage, topology = 'corners'):
        ...
        for i in range(drone_number):
            for j in range(i):
                ...
                net = 0 if distance > max_range else 2 * random.randint(min_net_bandwidth, min_net_bandwidth + int(rst_net_bandwidth * (max_range - distance) / max_range))
                dn.net[i][j] = net
                dn.net[j][i] = net
    ...
    def lookForChainSolution(self, first = True, mode = 'any'):
        ...
        for node in range(self.drone_number + 1):
            solutionNode[0] = node
            resultFound, firstStop = self.recursiveLookForChainSolution(1, solutionNode, solutionFlow, results, self.drone_number + 1 if node == self.drone_number else self.drone_number, first, firstStop, mode)
            if resultFound:
                if first:
                    break
    ...
    def recursiveLookForChainSolution(self, index, solutionNode, solutionFlow, results, nextBase, first, firstStop, mode = 'bestNodes'):
            ...
            for node in range(self.drone_number + 1 if nextBase < self.node_number()-1 else self.drone_number):
                ...
                if node not in solutionNode:
                    ...
                    if self.net[solutionNode[index-1]][node] >= nextBandwidth:
                        ...
                        resultFound, firstStop = self.recursiveLookForChainSolution(index+1, solutionNode, solutionFlow, results, nextBase, first, firstStop, mode)
                        if resultFound:
                            return True
    ...
    def printSolution(self):
        for baseIndex in range(self.drone_number, self.node_number()):
            ...
            for solbaseIndex, solbaseNode in enumerate(self.chainsOrder[baseIndex]):
                print(" <--" + str(self.chainsFlows[baseIndex][solbaseIndex]) + "/" + str(self.net[solbasePrevNode][solbaseNode]) + "-- " + str(self.longlabels[solbaseNode]), end="")
...
class ExactSolution:
    ...
    def print(self, showAll = False, draw = False) :
        print('==========================================')
        if self.found:
