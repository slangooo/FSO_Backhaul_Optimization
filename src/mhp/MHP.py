...
class DroneNet:
    ...
    @staticmethod
    def createRandomAreaRelative(size, base_number, drone_number, max_bandwidth, net_bandwidth_percentage,
                                 max_range_percentage, topology='corners'):
        ...
        return DroneNet.createRandomArea(size, base_number, drone_number, max_bandwidth, net_bandwidth, max_range,
                                         topology)
        ...
        if mode == 'full':
            ...
            if self.found:
                ...
                if self.exact:
                    ...
                    print('  val {score} / {best} = {gaSol}%'.format(score=self.score, best=self.eaBest,
                                                                     gaSol=int(10000 * self.gaScoreFactor) / 100))
                    print('  top {gaTop} / {all} = {gaPos}%'.format(gaTop=self.gaTop, all=self.eaAll,
                                                                    gaPos=int(10000 * self.gaPositionFactor) / 100))
