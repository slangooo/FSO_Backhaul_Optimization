...
class DroneNet:
    ...
    #                           % of : drone_number * max_bandwidth/2 (=avg_bandwidth) / 4
    # max_range_percentage    : max range for connection between drones
    #                           % of : size
    # topology                : default 'corners', other options 'center'
    @staticmethod
    def createRandomArea(size, drone_number, max_bandwidth, net_bandwidth_percentage, max_range_percentage,
                         topology='corners'):
        dn = DroneNet(drone_number, 4)

        net_bandwidth = int(drone_number * max_bandwidth * net_bandwidth_percentage / 800)
        ...
        if (topology == 'corners'):
            dn.pos[drone_number + 0] = [0, 0]
            dn.pos[drone_number + 1] = [0, size]
            dn.pos[drone_number + 2] = [size, 0]
            dn.pos[drone_number + 3] = [size, size]
        elif (topology == 'center'):
            dn.pos[drone_number + 0] = [size / 2, size / 2]
            dn.pos[drone_number + 1] = [size / 2, size / 2]
            dn.pos[drone_number + 2] = [size / 2, size / 2]
            dn.pos[drone_number + 3] = [size / 2, size / 2]
        else:
            dn.pos[drone_number + 0] = [random.randint(0, size), random.randint(0, size)]
            dn.pos[drone_number + 1] = [random.randint(0, size), random.randint(0, size)]
            dn.pos[drone_number + 2] = [random.randint(0, size), random.randint(0, size)]
            dn.pos[drone_number + 3] = [random.randint(0, size), random.randint(0, size)]
        ...
        for i in range(drone_number):
            for j in range(i):
                ...
                net = 0 if distance > max_range else 2 * random.randint(min_net_bandwidth, min_net_bandwidth + int(
                    rst_net_bandwidth * (max_range - distance) / max_range))
                dn.net[i][j] = net
                dn.net[j][i] = net

        for i in range(drone_number, drone_number + 4):
    ...
    def lookForChainSolution(self, first=True, mode='any'):
        ...
        for node in range(self.drone_number + 1):
            solutionNode[0] = node
            resultFound, firstStop = self.recursiveLookForChainSolution(1, solutionNode, solutionFlow, results,
    ...
    def recursiveLookForChainSolution(self, index, solutionNode, solutionFlow, results, nextBase, first, firstStop,
                                      mode='bestNodes'):
        if index < (self.node_number() - 1):
            ...
            for node in range(self.drone_number + 1 if nextBase < self.node_number() - 1 else self.drone_number):
                ...
                if node not in solutionNode:
                    nextBandwidth = solutionFlow[index - 1] + self.bandwidth[solutionNode[index - 1]] if solutionNode[
                                                                                                             index - 1] < self.drone_number else 0
                    if self.net[solutionNode[index - 1]][node] >= nextBandwidth:
                        solutionNode[index] = node
                        solutionFlow[index] = nextBandwidth
                        resultFound, firstStop = self.recursiveLookForChainSolution(index + 1, solutionNode,
    ...
    def printSolution(self):
        for baseIndex in range(self.drone_number, self.node_number()):
            ...
            for solbaseIndex, solbaseNode in enumerate(self.chainsOrder[baseIndex]):
                print(" <--" + str(self.chainsFlows[baseIndex][solbaseIndex]) + "/" + str(
                    self.net[solbasePrevNode][solbaseNode]) + "-- " + str(self.longlabels[solbaseNode]), end="")
                solbasePrevNode = solbaseNode
            print()

    def print(self):
        print('==========================================')
        print('drone_number               = ', self.drone_number)
        print('node bandwidth min/avg/max = ', numpy.min(self.bandwidth), '/', round(numpy.average(self.bandwidth)),
              '/', numpy.max(self.bandwidth))
        net_bandwidth = []
        for i in range(0, self.node_number()):
            for j in range(i + 1, self.node_number()):
                if (self.net[i][j] > 0):
                    net_bandwidth.append(self.net[i][j])
        print('net  bandwidth min/avg/max = ', numpy.min(net_bandwidth), '/', round(numpy.average(net_bandwidth)), '/',
              numpy.max(net_bandwidth))
        node_distance = []
        node_range = []
        ...
        print('net  distance  min/avg/max = ', numpy.min(node_distance), '/', round(numpy.average(node_distance)), '/',
              numpy.max(node_distance))
        print('net  range     min/avg/max = ', numpy.min(node_range), '/', round(numpy.average(node_range)), '/',
              numpy.max(node_range))
        print('------------------------------------------')
    ...
    def draw(self, showSolution=True, fileName='final_res.png'):
        ...
        if (showSolution):
            for baseIndex in range(self.drone_number, self.node_number()):
                ...
                for chainIndex, nodeIndex in enumerate(self.chainsOrder[baseIndex]):
                    ...
                    edge_labels[key] = str(self.chainsFlows[baseIndex][chainIndex]) + "/" + str(
                        self.net[prevNodeIndex][nodeIndex])
...
class ExactSolution:
    ...
    def print(self, showAll=True, draw=True):
        print('==========================================')
        if self.found:
            print('time F : ', int(self.firstTime), 's')
            print('time T : ', int(self.fullTime), 's')
...
class GASolution:
    ...
    def print(self, exact: ExactSolution = None, draw=True, drawFileName='ga_sol.png', plotFitness=True):
        print('==========================================')
        ...
        if self.found:
            ...
            if exact is not None:
                gaSol = 100 * self.score / exact.bestScore
                gaTop = 0
                for result in exact.results:
                    score = result[1]
                    if score > self.score:
                        gaTop += 1
                lenResults = len(exact.results)
                gaPos = 100 * (lenResults - gaTop) / lenResults

                print('GA versus Exact')
                print('  val {score} / {best} = {gaSol}%'.format(score=self.score, best=exact.bestScore,
                                                                 gaSol=int(100 * gaSol) / 100))
                print('  top {gaTop} / {all} = {gaPos}%'.format(gaTop=gaTop, all=lenResults,
                                                                gaPos=int(100 * gaPos) / 100))
...
class GenAlg:
    def __init__(
            ...
            fitnessChainMode='bestNodes',
            # any : For all correct return 1; bestEdges : For correct return sum of surplus on edges; bestNodes : For correct return sum of surplus on nodes
            parent_selection_type='sss',
            # sss : steady_state_selection, rws : roulette_wheel_selection, sus : stochastic_universal_selection, random, tournament, rank
    ...
    def run(self, dn: DroneNet) -> GASolution:
        ...
        while ((self.timeLimit <= 0) or (self.timeLimit > executionTime)) and (
                (not self.anySolution) or (solutionFitness < 0)):
            ga = GA(
                ...
                fitness_func=lambda solution, solution_idx: dn.fitnessChain(solution, solution_idx,
                                                                            self.fitnessChainMode),

                parent_selection_type=self.parent_selection_type,
                crossover_type=lambda parents, offspring_size, ga: dn.singleChainCrossover(parents, offspring_size, ga,
                                                                                           self.crossoverFix),
