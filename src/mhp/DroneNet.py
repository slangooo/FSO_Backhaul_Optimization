...
class DroneNet:
    ...
    def lookForRandomChainSolution(self, stopOnFirst = True, time_limit = 0, instance_limit = 0, fitness_mode = 'nodeSurplus', penalty_mode = 'penalty', penalty_value = 0):
        start_time = time.time()
        stop_type = 'time'
        results = []
        processed = 0
        if time_limit == 0 and instance_limit == 0:
            time_limit = 1
        end_time = start_time + time_limit
        while True:
            if time_limit and time.time() > end_time:
                stop_type = 'time'
                break;
            ...
            if solution:
                ...
                if score >= 0:
                    results.append((solution.copy(), score, time.time() - start_time, processed))
                    if stopOnFirst:
                        stop_type = 'first'
                        break
            processed += 1
    ...
    def lookForChainSolution(self, stopOnFirst = True, mode = 'nodeSurplus', time_limit = 0, instance_limit = 0):
        stop_type = 'end'
        results = []
        start_time = time.time()
        ...
        for node in range(self.drone_number + 1):
            ...
            stop_type, force_stop, instance_count = self.recursiveLookForChainSolution(
                    ...
                    stopOnFirst, start_time, time_limit, instance_count, instance_limit, mode)
            if force_stop:
                break
    ...
    def recursiveLookForChainSolution(
            ...
            nextBase,
            stopOnFirst, start_time, time_limit, instance_count, instance_limit,
            mode = 'nodeSurplus'):
        if index < (self.node_number() - 1):
            ...
            if self.net[solutionNodes[index-1]][node] >= nextBandwidth:
                score = self.fitnessChain(
                    ...
                    0,
                    mode)
                results.append((solutionNodes.copy(), score, time.time() - start_time, instance_count))
                if stopOnFirst:
    ...
    def randomSolutions(self, size, allow_missing_edges = True):
        solutions = []
        for sol_i in range(size):
            solution = self.randomSolution(allow_missing_edges = allow_missing_edges)
            if solution:
                solutions.append(solution)
        
        return solutions

    def randomSolution(self, allow_missing_edges = True):
